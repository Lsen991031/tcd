...
class HypercorrSqueezeNetwork(nn.Module):
    def __init__(self, backbone, use_original_imgsize):
        ...
        def forward(self, old_features, new_features):
            with torch.no_grad():
                corr = Correlation.multilayer_correlation(old_features, new_features, self.stack_ids)
    ...
    #     return logit_mask
    def forward(self, input, old_model, model):
        with torch.no_grad():
            old_feats = old_model.module.base_model.extract_feat_res(input.view((-1, 3) + input.size()[-2:]))
            new_feats = model.module.base_model.extract_feat_res(input.view((-1,3) + input.size()[-2:]))
            corr = Correlation.multilayer_correlation(old_feats, new_feats, self.stack_ids)

        logit_mask = self.hpn_learner(corr)
        if not self.use_original_imgsize:
            logit_mask = self.hpn_learner
